    ...
    <div className="flex items-center justify-center min-h-screen px-6 py-12 bg-[#080b11] text-slate-100 relative overflow-hidden">
      ...
      >
        <div className="flex flex-col items-center mb-8">
          <div className="flex items-center justify-center w-14 h-14 bg-gradient-to-tr from-blue-600 to-indigo-600 text-slate-100 rounded-2xl mb-4 shadow-lg shadow-blue-500/20">
            <Shield className="w-7 h-7 text-white" />
          </div>
          <h2 className="text-2xl font-extrabold text-slate-100 tracking-tight">MediCore 360</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-11 pr-4 py-3 bg-slate-950/50 border border-slate-800 rounded-xl text-slate-100 placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500/80 transition-all duration-300"
              />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="••••••••••••"
                className="w-full pl-11 pr-11 py-3 bg-slate-950/50 border border-slate-800 rounded-xl text-slate-100 placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500/80 transition-all duration-300"
              />
              <button
                type="button"
          ...
          <AnimatePresence mode="wait">
            {error && (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: -10 }}
                className="p-3.5 bg-red-950/30 border border-red-900/50 rounded-xl text-xs text-red-400 font-medium"
              >
                {error}
              </motion.div>
            )}
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.99 }}
            className="w-full py-3.5 px-4 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white font-semibold rounded-xl flex items-center justify-center space-x-2 shadow-lg shadow-blue-950/50 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {loading ? (
              <>
        ...
        <div className="mt-8 text-center text-xs text-slate-500 border-t border-slate-800/60 pt-6">
          MediCore 360 utilizes cryptographic salts, application hashing keys (Chili Pepper) and SMTP-driven multi-factor tokens to safeguard enterprise patient records.
        </div>
      </motion.div>
    </div>
  );
