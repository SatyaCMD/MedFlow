  ...
  const columns = [
    {
      ...
      accessor: (row: PatientInvoice) => (
        <button
          ...
          className="font-mono font-black text-blue-600 hover:text-blue-800 underline text-xs cursor-pointer whitespace-nowrap"
        >
          {row.invoiceCode}
        </button>
      ),
    ...
    },
    {
      header: 'Line Items',
      accessor: (row: PatientInvoice) => (
        <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 bg-slate-100/90 border border-slate-200 text-slate-700 font-extrabold rounded-full text-[10px] whitespace-nowrap shadow-2xs">
          <span className="w-1.5 h-1.5 rounded-full bg-blue-600 shrink-0" />
          <span>{row.lineItems.length} {row.lineItems.length === 1 ? 'Charge' : 'Charges'}</span>
        </span>
      ),
    },
    {
