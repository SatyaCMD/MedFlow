...
export const AmbulanceManagementModule: React.FC = () => {
  ...
  const isPatientRole = userRoleStr === 'PATIENT' || userRoleStr === 'USER' || !userRoleStr;
      ...
      {activeTab === 'DISPATCH' && (
        <div className="space-y-6">
          <div className="p-6 bg-slate-50 border border-slate-200 rounded-3xl space-y-4">
            ...
            {/* PATIENT-ONLY VIEW vs STAFF/ADMIN FLEET QUEUE */}
            {isPatientRole ? (
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-black uppercase tracking-wider text-slate-500">
                    My Booked Emergency Ambulance & Live Telemetry
                  </span>
                  <span className="px-2.5 py-0.5 bg-rose-100 border border-rose-300 text-rose-900 text-[10px] font-black rounded-full uppercase">
                    1 ACTIVE DISPATCH BOOKING
                  </span>
                </div>
                ...
                {patientBookedAmbulance ? (
                  <div className="p-6 bg-white border-2 border-rose-300 rounded-3xl shadow-md max-w-xl space-y-4">
                    <div className="flex items-center justify-between">
                      <span className="px-3.5 py-1 bg-rose-100 text-rose-900 font-black text-xs rounded-full border border-rose-300 animate-pulse flex items-center gap-1.5">
                        <span className="w-2 h-2 rounded-full bg-rose-600 animate-ping" />
                        {patientBookedAmbulance.status}
                      </span>
                      <span className="text-xs font-mono font-black text-slate-600">Plate: {patientBookedAmbulance.vehiclePlate}</span>
                    </div>

                    <div>
                      <div className="flex items-center justify-between">
                        <h4 className="font-extrabold text-base text-slate-900">Driver: {patientBookedAmbulance.driverName}</h4>
                        <span className="text-xs font-black text-blue-700 bg-blue-50 px-2.5 py-1 rounded-full border border-blue-200">
                          {patientBookedAmbulance.unitType} ICU
                        </span>
                      </div>
                      <p className="text-xs font-semibold text-slate-500 mt-0.5">Emergency Contact: {patientBookedAmbulance.driverPhone}</p>
                    </div>

                    <div className="p-4 bg-slate-50 border border-slate-200 rounded-2xl text-xs space-y-2">
                      <div className="flex justify-between font-semibold">
                        <span className="text-slate-500">Telemetry Speed:</span>
                        <span className="font-bold text-slate-900 font-mono">{patientBookedAmbulance.currentSpeed} km/h</span>
                      </div>
                      <div className="flex justify-between font-semibold">
                        <span className="text-slate-500">GPS Live Location:</span>
                        <span className="font-black text-rose-600">{patientBookedAmbulance.lastLocation}</span>
                      </div>
                    </div>
                    ...
                    <div className="space-y-2 pt-1">
                      ...
                      >
                        ...
                        <span>View Real-Time Live Route Map</span>
                      </button>
                  ...
                  <div className="p-8 bg-white border border-slate-200 rounded-3xl text-center space-y-3">
                    <Siren className="w-10 h-10 text-rose-600 mx-auto animate-bounce" />
                    <h4 className="font-extrabold text-sm text-slate-900">No Active Ambulance Booking Found</h4>
